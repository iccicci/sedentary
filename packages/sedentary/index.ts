...
export class Sedentary<D extends DB<T>, T extends Transaction> {
  ...
  public model<A extends AttributesDefinition, P extends ModelStd, M extends Record<string,() => void>>(
    ...
    let aArray: Attribute<unknown, boolean, unknown>[] = int8id
      ? [new Attribute<bigint, true, unknown>({ ...this.Int8(), attributeName: "id", fieldName: "id", modelName, notNull: true, tableName, unique: true })]
      : [new Attribute<number, true, unknown>({ ...this.Int({ size: 4 }), attributeName: "id", fieldName: "id", modelName, notNull: true, tableName, unique: true })];
    ...
    for(const attributeName of Object.keys(_attributes).sort()) {
      ...
      let {
        ...
        const ret = new Attribute({ attributeName, fieldName: attributeName, modelName, notNull: false, tableName, unique: false, ...attributeDefinition });
